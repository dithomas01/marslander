var plotTerrain = function (terrainX, terrainY) {
    ...
    d3.select("#btn-startSim").style("display", "block");
    d3.select("#input-loopNumber").style("display", "block");
}

var plotChromosome = function (polyArray, crashArray) {
    console.log("plotChromosome");
    d3.select(".lineContainer").remove();
    d3.select("#div-actualLoop").style("display", "flex");
